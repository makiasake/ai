...
public class Test {
	...
	private void callTestScript() throws IOException, InterruptedException {
		new Util().runCommand(Arrays.asList(StringUtils.split(new Util().readProperty("test.script.path"), " ")));
	}
	...
	private static void callKokoroScript() throws IOException, InterruptedException {
		...
		new Util().runCommand(Arrays.asList(StringUtils.split(command, " ")));
	}

	private static void callKokoroToGenerateAudio() throws IOException, InterruptedException {
		final String command = "py -m uv run -m --directory=" + new Util().readProperty("kokoro.path") + " kokoro_tts "
				...
				+ " --speed 1.0 --lang en-us --voice af_sarah --format wav";
